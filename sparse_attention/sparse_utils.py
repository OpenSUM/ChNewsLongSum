...
class recomputable(object):
    ...
    def meta_fn(self, *args, **kwargs):
        ...
        try:
            ...
            args_str = ''.join([''.join([str(x) for x in a.shape.as_list() if x is not None]) for a in args])
            # print(args_str)
            # args_str = '0' if args_str == '' else args_str
            size_hash = str(hash(int(args_str)))[0:6]
    ...
    def build_fns(self, name, outer_args, outer_kwargs):
        ...
        def bwd(*args):
            ...
            # print('[bwd] args:', args)
            # with tf.device("/gpu:1"), tf.control_dependencies([dy]):

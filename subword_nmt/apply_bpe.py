...
def create_parser(subparsers=None):
    ...
    parser.add_argument(
        '--input', '-i', type=argparse.FileType(mode='r', encoding='utf-8'), default=sys.stdin,
        metavar='PATH',
        help="Input file (default: standard input).")
    parser.add_argument(
        '--codes', '-c', type=argparse.FileType(mode='r', encoding='utf-8'), metavar='PATH',
        required=True,
...
if __name__ == '__main__':
    ...
    for i, line in enumerate(args.input):
        if i % 1000 == 0:
            print('\r%d samples' % i, end='')
        js = json.loads(line.strip())
        js['title'] = bpe.process_line(js['title'])
        js['abstract'] = bpe.process_line(js['abstract'])
        js['text'] = bpe.process_line(js['text'])
        args.output.write(json.dumps(js, ensure_ascii=False))
        args.output.write('\n')
    print('Finish!')
        ...
        with tf.variable_scope("embeddings", reuse=tf.AUTO_REUSE):
            ...
            if encoder_output is not None:
                with tf.variable_scope('shift'):
                    start_embedding = encoder_output[:, 0, :]
        ...
        with tf.variable_scope(scope, default_name="bert", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("encoder"):
                ...
                self.all_encoder_layers, self.all_attention_probs, self.all_self_attention_probs = transformer_model(
                    ...
                    memory_tensor=None if encoder_output is None else encoder_output[:, :, :],
...
def attention_layer(from_tensor,
                    ...
                    to_seq_length=None,
                    trainable=True,
                    return_attention_probs=True,#False,
                    **kwargs):
...
def transformer_model(input_tensor,
    ...
    if is_decoding:
        memory_tensor_shape_list = get_shape_list(memory_tensor)
            ...
            if is_decoding:
                with tf.variable_scope("masked_attention"):
                    ...
                    with tf.variable_scope("self"):
                        attention_head, self_attention_prob = attention_layer(
                ...
                memory_tensor = layer_input  # if is not decoding, using self-attention.
            # end add.

            with tf.variable_scope("attention"):
                attention_heads = []
                with tf.variable_scope("self"):
                    func = attention_layer if is_decoding else my_attention_layer
                    output = func(
                        from_tensor=layer_input,
                        to_tensor=memory_tensor,  # we modify here. change layer_input to memory_tensor.
                        ...
                        masked=False)
                    if is_decoding:
                        attention_head, attention_prob =  output  
                        all_layer_attention_probs.append(attention_prob)
                    else:
                        attention_head =  output  

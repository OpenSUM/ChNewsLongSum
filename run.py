...
def test_batch(model, sess, eval_batcher, seq_length, word2id, id2word, use_pointer, substr_prefix, verbose=False, **kwargs):
    ...
    for batch_index, batch_data in enumerate(eval_batcher.batch(), 1):
        ...
        encoder_output = sess.run(model.encoder_output, feed_dict=fd)
        encoder_output = np.array_split(encoder_output, model.num_gpus, axis=0)
        ...
        for i in range(seq_length):
            ...
            fd = model.get_decode_decoder_feed_dict(batch_data=batch_data, encoder_output=encoder_output)
...
def train(FLAGS):
    ...
    eval_batcher = get_batcher(src_file=config.eval_src,
                               dst_file=config.eval_src,
                               word2id=word2id, src_seq_length=config.encoder_seq_length,
                               dst_seq_length=config.decoder_seq_length, batch_size=config.batch_size,
                               do_lower=config.do_lower,
                               substr_prefix=config.substr_prefix,
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        best_r2_epoch = max(0, start_epoch)
        print('Run global_variables_initializer()')
        sess.run(tf.global_variables_initializer())
        best_r2_global_step = sess.run(model.global_step)
        ...
        for epoch_index in range(start_epoch, config.epochs):
            ...
            for batch_index, batch_data in enumerate(train_batcher.batch(), 1):
                ...
                if global_step % CHECK_GLOBAL_STEPS == 0 and global_step != 0 and \
                    ...
                    scores, loss, _ = test_batch(model=model, sess=sess, eval_batcher=eval_batcher,
                                                 seq_length=config.decoder_seq_length, word2id=word2id, id2word=id2word,
                                                 use_pointer=config.use_pointer, substr_prefix=config.substr_prefix)
                    if scores['rouge-2']['f'] <= best_r2_for_step \
                            and global_step - best_r2_global_step >= HALVE_LR_STEPS:
                        if HALVE_BERT_LR and bert_learning_rate >= MIN_LEARNING_RATE:
                            bert_learning_rate /= 2
                        if HALVE_OTHER_LR and other_learning_rate >= MIN_LEARNING_RATE:
                            other_learning_rate /= 2
                        best_r2_global_step = global_step
                    elif scores['rouge-2']['f'] > best_r2_for_step:
                        best_r2_for_step = scores['rouge-2']['f']
                        best_r2_global_step = global_step

                    print(('\tBest Rouge-2: {:.8}, current Rouge-1: {:.8}, current Rouge-2: {:.8}, '
                           'current Rouge-L: {:.8}. Current bert lr: {:.8}, other lr: {:.8}').format(
                        best_r2_for_step, scores['rouge-1']['f'], scores['rouge-2']['f'], scores['rouge-l']['f'],
                        bert_learning_rate, other_learning_rate))

            # scores, loss, texts = test_batch_when_training(model=model, sess=sess, eval_batcher=eval_batcher,
            #                                  seq_length=config.decoder_seq_length, id2word=id2word,
            #                                  use_pointer=config.use_pointer, substr_prefix=config.substr_prefix,
            #                                  beam_size=2)
            # print('Eval: Rouge-1:{:.2f}, Rouge-2:{:.2f}, Rouge-L:{:.2f}'.format(
            #     scores['rouge-1']['f'], scores['rouge-2']['f'], scores['rouge-l']['f']))
            # saver.summary(loss=loss, scores=scores, prefix='eval', global_step=epoch_index)
            # saver.save_summaries(sources=texts['source'], refs=texts['ref'], cands=texts['cand'], step=epoch_index)
            if not config.debug:
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string(
        # "bert_config_file", './bert/lcsts/bert_config.json',
        # "bert_config_file", './bert/cnndaily/bert_config.json',
        # "bert_config_file", './bert/cnndaily/transformer.json',
        "bert_config_file", './bert/news/bert_config.json',
